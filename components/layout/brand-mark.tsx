    ...
    <span className="flex items-center gap-3">
      ...
      {showText ? (
        <span className="min-w-0 text-right">
          <span className="block text-[10px] tracking-[0.18em] text-gold uppercase">Galaxy</span>
          <span className={cn("mt-0.5 block text-sm font-semibold leading-5", textMain)}>المجرة</span>
          <span className={cn("block text-[11px] leading-4", textMuted)}>{siteConfig.productName}</span>
        </span>
      ) : null}
    </span>
  );
