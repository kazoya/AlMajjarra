...
function parse(raw: string): AssessmentValues {
  ...
  try { return { ...defaultAssessment, ...JSON.parse(raw) }; } catch { return defaultAssessment; }
}
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <Card className="shadow-sm">
        <CardHeader>
          <CardTitle className="text-base">{summary.filled} من {summary.total} ({summary.pct}٪)</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2 text-sm leading-7 text-muted-foreground">
          ...
          <p>حساسات: {values.sensorsNow.join(" · ") || "غير محدد"}</p>
          <p>الري الحالي: {values.irrigationMethod || "غير محدد"}</p>
          <p>ووتر سيفر: {values.waterSaverUse || "غير محدد"}</p>
          <p>ملاحظات: {values.notes || "لا شيء"}</p>
        </CardContent>
      </Card>
      <Button asChild className="cursor-pointer"><Link href="/assessment">تعديل</Link></Button>
    </div>
  );
}
